...
abstract class APaypal extends \XLite\Model\Payment\Base\Iframe
{
    ...
    public function processReturn(\XLite\Model\Payment\Transaction $transaction)
    {
        ...
        if (\XLite\Core\Request::getInstance()->cancel) {
            ...
            if ($request->isPost() && isset($request->RESULT) && 0 < intval($request->RESULT)) {
    ...
    protected function doCapture(\XLite\Model\Payment\BackendTransaction $transaction)
    {
        ...
        if (!empty($responseData)) {
            ...
            if ('0' == $responseData['RESULT']) {
                ...
                $transaction->getPaymentTransaction()->getOrder()->setPaymentStatus(\XLite\Model\Order\Status\Payment::STATUS_PAID);
    ...
    protected function doVoid(\XLite\Model\Payment\BackendTransaction $transaction)
    {
        ...
        if (!empty($responseData)) {
            ...
            if ('0' == $responseData['RESULT']) {
                $result = true;
                $status = \XLite\Model\Payment\Transaction::STATUS_SUCCESS;
                $transaction->getPaymentTransaction()->getOrder()->setPaymentStatus(\XLite\Model\Order\Status\Payment::STATUS_DECLINED);
    ...
    protected function getCreateSecureTokenRequestParams()
    {
        ...
        $postData = array(
            ...
            'INVNUM'            => $this->getSetting('prefix') . $order->getOrderNumber(),
    ...
    protected function getLineItems($cart = null)
    {
        ...
        if (!empty($items)) {
            ...
            foreach ($items as $item) {
                $lineItems['L_COST' . $index] = $obj->getCurrency()->roundValue($item->getItemNetPrice());
