...
CommonElement.prototype.handlers.push(
  {
    ...
    handler: function() {
      ...
      $wrapper.model_selector_options = {
      ...
      this.$element.bind('keyup', function (event) {
        if (
          $element.val().length >= $wrapper.model_selector_options.min_count
          ...
          if (!isModelSelected()) {
            ...
            core.get(
              $wrapper.model_selector_options.getter + '&search=' + jQuery(this).val(),
        ...
        if ($element.val().length < $wrapper.model_selector_options.min_count) {
          if ($element.val().length > 0) {
            jQuery('.enter-more-characters', $wrapper).html(core.t(
              'Enter X more characters to start search', {
                X: $wrapper.model_selector_options.min_count - $element.val().length
            })).removeClass('hidden');
          } else {
            jQuery('.enter-more-characters', $wrapper).addClass('hidden');
          }
