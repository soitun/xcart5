<?php
// vim: set ts=4 sw=4 sts=4 et:

/**
 * X-Cart
 *
 * NOTICE OF LICENSE
 *
 * This source file is subject to the software license agreement
 * that is bundled with this package in the file LICENSE.txt.
 * It is also available through the world-wide-web at this URL:
 * http://www.x-cart.com/license-agreement.html
 * If you did not receive a copy of the license and are unable to
 * obtain it through the world-wide-web, please send an email
 * to licensing@x-cart.com so we can send you a copy immediately.
 *
 * DISCLAIMER
 *
 * Do not modify this file if you wish to upgrade X-Cart to newer versions
 * in the future. If you wish to customize X-Cart for your needs please
 * refer to http://www.x-cart.com/ for more information.
 *
 * @category  X-Cart 5
 * @author    Qualiteam software Ltd <info@x-cart.com>
 * @copyright Copyright (c) 2011-2014 Qualiteam software Ltd <info@x-cart.com>. All rights reserved
 * @license   http://www.x-cart.com/license-agreement.html X-Cart 5 License Agreement
 * @link      http://www.x-cart.com/
 */

namespace XLite\Module\CDev\XPaymentsConnector\View\Settings;

/**
 * Test module
 *
 * @ListChild (list="admin.center", zone="admin", weight="10")
 */
class Tabs extends \XLite\Module\CDev\XPaymentsConnector\View\Settings\ASettings
{
    /**
     * Return navigation tabs
     *
     * @return array
     */
    public function getTabs()
    {
        if (\XLite\Module\CDev\XPaymentsConnector\Core\XPaymentsClient::getInstance()->isModuleConfigured()) {

            $tabs = array(
                \XLite\Module\CDev\XPaymentsConnector\Core\Settings::SECTION_PAYMENT_METHODS => array(
                    'title' => 'Payment methods',
                ),
                \XLite\Module\CDev\XPaymentsConnector\Core\Settings::SECTION_CONNECTION => array(
                    'title' => 'Connection',
                ),
                \XLite\Module\CDev\XPaymentsConnector\Core\Settings::SECTION_ZERO_AUTH => array(
                    'title' => 'Save credit card setup',
                ),
                \XLite\Module\CDev\XPaymentsConnector\Core\Settings::SECTION_MAP_RULES => array(
                    'title' => 'Order status mapping rules',
                ),
            );

        } else {
        
            // Prepend welcome page
            $tabs = array(
                \XLite\Module\CDev\XPaymentsConnector\Core\Settings::SECTION_WELCOME => array(
                    'title' => 'Welcome',
                ),
                \XLite\Module\CDev\XPaymentsConnector\Core\Settings::SECTION_CONNECTION => array(
                    'title' => 'Connection',
                ),
            );
        }

        foreach ($tabs as $key => $tab) {
            $tabs[$key]['url'] = \XLite\Core\Converter::buildUrl(
                'xpc',
                '',
                array(
                    'section' => $key
                )
            );
        }

        if ($tabs[\XLite\Core\Request::getInstance()->section]) {
            $tabs[\XLite\Core\Request::getInstance()->section]['selected'] = true;
        }

        return $tabs;
    }

    /**
     * Return widget default template
     *
     * @return string
     */
    protected function getDefaultTemplate()
    {
        return $this->getDir() . '/tabs.tpl';
    }

    /**
     * List of tabs/sections where this setting should be displayed
     *
     * @return boolean
     */
    public function getSections()
    {
        return \XLite\Module\CDev\XPaymentsConnector\Core\Settings::getAllSections();
    }
}
