    ...
    public function doTransactionRequest($issuerId, $transid)
    {
        if ($issuerId) {
            ...
            if (!$this->transaction && $transid) {
                $this->transaction = \XLite\Core\Database::getRepo('XLite\Model\Payment\Transaction')->find($transid);
            }
            ...
            if ($this->transaction) {
                ...
                $orderId = $this->getSetting('prefix') . $this->transaction->getTransactionId();
                ...
                $transRequest->setOrderDescription('Order #' . $this->transaction->getTransactionId());
                $transRequest->setEntranceCode($this->transaction->getTransactionId());
